...
- [Purpose](#purpose)
    - [Description](#description)
      ...
      - [The Title display](#the-title-display)
          - [The BankAccountStatement.AddTitle method](#the-bankaccountstatementaddtitle-method)
          - [The BankAccountStatement.AddTitleWithBankName method](#the-bankaccountstatementaddtitlewithbankname-method)
          - [The BankAccountStatement.AddTextTitle method](#the-bankaccountstatementaddtexttitle-method)
          - [The BankAccountStatement.AddStatementInfoToTitle method](#the-bankaccountstatementaddstatementinfototitle-method)
          - [The BankAccountStatement.AddLogo method](#the-bankaccountstatementaddlogo-method)
      ...
      - [8. The BankAccountStatementSecondPageBuilder class](#8-the-bankaccountstatementsecondpagebuilder-class)
          - [The Title display](#the-title-display-1)
          - [The BankAccountStatement.AddCommonPageTitle method](#the-bankaccountstatementaddcommonpagetitle-method)
          ...
          - [The AddStatementsHead method](#the-addstatementshead-method-1)
...
The second and following page(s) contain a table with transaction history and tables with service fees. The pages consist of 
...
The number of following pages, including the last page, depends on an amount of data in a the transaction history. The pages are added automatically without changing the code because the data is uploaded from an external file, and the library manages the addition of  the pages.
...
* Separation of responsibility. Class Program processes command line options, possible errors during document creation and displays a report of the errors. The rest of the classes do not process the command line options and errors. They are used only to build the document.
* Reduction of responsibility of each class. This can help to limit the size of the code for each class that improves the understanding of the code example.
* The document consists of several pages, each class  builds its own page.

#### 4. Class Program
Responsibility:
* Parsing command line options.
It processes the command line options using the **PrepareParameters** that prepares the **Parameters** structure.
* Displaying prompts to user when one of the following options is presented on the command line: “?”, “-h”, “-help”, “--h”, “--help”. It is processed by the  **Usage**.
* Building document.
It delegates the document building to the **BankAccountStatementRunner** class with the name of the resulting document file.
* Running the application to demonstrate the resulting document if it is specified in the command line options.

#### 5. The BankAccountStatementRunner class
The **BankAccountStatementRunner.Run** performs the responsibilities to
...
This data can be received from an external source, such as a database, in the real application.

##### The *sample-statement-data.json* file and Model.Statement class

This file contains a json array of all operations performed on user's account for the given period. Items of the array are json objects, each of which contains the data of one operation:
...
The **BankAccountStatementRunner.Build** converts the json object into the object of the **Model.StatementInfo** class:
...
Responsibility to
* Save the list of the **Model.Statement** objects in the **statement** property; 
* Save the object of the **Model.StatementInfo** in the **statementInfo** property.

The **Build** performs the responsibilities to:

* Create the object of the **Gehtsoft.PDFFlow.Builder.DocumentBuilder** class;
* Create the object of the **BankAccountStatementFirstPageBuilder** class, initialize it with **statementInfo** property and build the first page using the **Build** method;
* Create the object of the **BankAccountStatementFirstPageBuilder** class, initialize it with the **statementInfo** property and the **statement** property  and build the other page(s) using the **Build** method;
* Create the object of the **BankAccountStatementLastPageBuilder** class, initialize it with **statementInfo** property  and build the last page using the **Build** method;
...
The **BankAccountStatementFirstPageBuilder.Build method** performs the responsibilities to:
...
The Title on the first page (See Fig. 3) is different from the Title on the other pages. 
...
The method adds the table with one row and 2 columns consisted of
* The Title text on the left column
* The bank logo on the right column 
...
The **BankAccountStatementFirstPageBuilder.Build** method passes the **BankAccountStatement.AddTitleWithBankName** method as the **AddTitleAction** parameter in order to add the Bank Name to the common Title (See Fig.5)

###### The BankAccountStatement.AddTitleWithBankName method

There must be the common text containing the account number, start and end dates of the account statement, page numbers in the Title. 
The method adds the paragraph with the Bank Name and calls the **BankAccountStatement.AddTextTitle** method that adds the common text of the Title.
...
The paragraph is added using the **Helper.AddParagraph** method that has the parameter of the **Gehtsoft.PDFFlow.Builder.TableCellBuilder** class.
The static class of **Helper** was created to avoid a code duplication. The methods of the class will be described in the end of the article. 

###### The BankAccountStatement.AddTextTitle method

In order to add the account number, the start and end dates of the account statement, and the page numbers according to the design, the method creates the table with five columns and one row. 
The data location is regulated by the width of the table and the width of the columns.
...
* Creates the table with the full width of the page. The table consists of 2 columns with widths 62% and 38% of the page width.
* Adds one row to the table.
* Creates 2 cells in the row and sets the right border for the left cell.
* Calls the actions to fill the left and right cells.

The method takes the parameters: 
* The **SectionBuilder** - an object of the **Gehtsoft.PDFFlow.Builder.SectionBuilder** class of the section where 2 blocks are added; 
...
The method to fill the table by the data of the account summary
...
```csharp
                .SetColSpan(2)
```
* Fills the cell by the title text;
```csharp
                .AddParagraphToCell("Activity summary");
```
* Fills the rows of the table using the **FillActivitySummaryTableRow** method by passing the **Field** array with the data of the account summary to the table.  
```csharp
...
Therefore, the method first defines 4 styles. Then, the method adds the rows applying the style and using the **TableCellBuilder.SetPadding** for the last line in the **for** cycle through the array. 
...
* Creates the table with the full width of the page. The table consists of one row and 2 columns with widths 63% and 37% of the page width.
* Adds 2 cells to the row.
* Sets the top borders for the cells to display lines above them. In order to add a gap between the lines on the top, the method sets the left border for the right cell. The left border is set up with 10 pt. width and white color. The color is not visible and separates the borders between the cells.
...
The method adds the checkboxes to the table's cell.
To align the checkboxes to the right of the cell, the method adds the nested table to the cell.
The method creates columns 60% width for the label and 40% width for the checkbox in the table.
The method adds one row for each checkbox, adds the label rows and the checkbox symbols to the cells in the **foreach** cycle. 
...
The **BankAccountStatementSecondPageBuilder.Build** method performs responsibilities to :
* Create the page and set its parameters;
...
```
* Add the line separating the header from the rest of the page content;
```csharp
            headerBuilder.AddLine(PageWidth, 2f, Stroke.Solid);
```
* Add the blocks of "Overdraft protection", "Transaction history", "Service fee summary", "Transaction fee summary".
```csharp
...
The **BankAccountStatementSecondPageBuilder.Build** method passes the **BankAccountStatement.AddCommonPageTitle** method as the **AddTitleAction** parameter in order to add the common *Title*.

###### The BankAccountStatement.AddCommonPageTitle method

It calls the **BankAccountStatement.AddTextTitle** method to add the common text to the *Title* with the 10 pt. top margin. 
```csharp
...
The method adds the row for each object of the **Model.Statement** class to the *Statements* table. The objects are in the list in the **statements** property.  The values of each **Model.Statement** object from the list are used to fill the cells of one row of the table.

The empty values of 0 and null must not be shown.
To do this, the value of each property is converted depending on its type. The conversion is performed using the methods:
...
When the rows are added, the properties of the **total** parameter are changed 
to accumulate the values of the **Deposits** and **Withdrawals** properties.
...
The method adds 2 last rows to the *Statements* table
...
* The second row contains 2 values in the "Deposits/Credits" и "Withdrawals/Debits" columns. These values contain in the **total** of the **Deposits** and **Withdrawals** properties.
The **total** parameter is passed to the method.
...
The method adds the "*The Ending Daily Balance does not reflect...*" text below the table using the **Helper.AddParagraph** method that takes the object of the **Gehtsoft.PDFFlow.Builder.SectionBuilder** class as the parameter. 
...
The last row of the table is not included in the second page due to the number of items in the *Statements* list. 
The library creates the additional page automatically. Also, it adds the Title that has been added using the **Gehtsoft.PDFFlow.Builder.SectionBuilder.AddHeaderToBothPages** method.
Therefore, the sample program does not need to control the creation of additional pages.



###### The AddServiceFeeSummaryParagrpaph method

The method adds to the page: 
* The paragraph with the "*Monthly service fee summary*" title;
* The paragraph with text.

The method sets the paragraph since the URL is added to it.
...
The method creates the table with 4 columns. The last column is for the checkbox in the "Average ledger balance " row. 
The rows are added to the table using the **AddTableRows** method that is passed the rows as the array of the objects of the **RowData** inner class. The rows are created using the **GetServiceFeeSummaryRowData** method.
...
The object of this class contains
* The **data** array of the strings;
* The **font** array of fonts with one font for each string.

Also, the class defines:

* The **virtual** **GetLength** method using to define the number of the rows;
* The **virtual** **AddTextToParagraph** method using to add the rows to the table;
...
The method creates the objects of the **RowData** array with the values for the *ServiceFeeSummary* table using the **statementInfo** property.
...
The method adds the rows to the *ServiceFeeSummary* table using the passed array of the objects of the **RowData** class. 
...
The block consists of 
* the line;
* the paragraph with the "*Account transaction fees summary*" title;
* the fee table;
* the line;
* the paragraph with the "*Your feedback matters...*" checkbox.
...
* Adds the line using the **Gehtsoft.PDFFlow.Builder.SectionBuilder.AddLine** method;
* Adds the paragraph with title using the **Helper.AddParagraph** method that takes the object of the **Gehtsoft.PDFFlow.Builder.SectionBuilder** class as the parameter.


###### The AddTransactionFeeSummaryTable method

The method creates the table with 6 columns.
The rows are added to the table using the **AddTableRows** method that is passed the rows as the array of the objects of the **RowData** inner class. The rows are created using the **GetTransactionFeeSummaryRowData** method.

The row numbers with bottom border is the last parameter passed to the **AddTableRows** method. This is #0 row. 
...
The method creates the objects of the **RowData** array with the values for the *TransactionFeeSummary* table using the **statementInfo** property.
...
The method adds the table with 2 rows to the page.
The checkbox is added to the left column.
...
The **BankAccountStatementLastPageBuilder.Build method** performs responsibilities to
* Create the page and set its parameters;
...
```csharp
            headerBuilder.AddLine(PageWidth, 2f, Stroke.Solid);
```
* Create the footer "©2020 SampleBank , N.A. All rights reserved. Member FDIC.";
```csharp
...
```
* Add the line separating the "**Polices**" from the rest of the page content;
```csharp
...
When the method adds the table filled by the **FillGeneralStatementPoliciesTable** method.
...
The method adds 2 columns and one row to the table and adds the **Polices** text to 2 cells of the row.
...
The method adds the paragraph with the "*Account Balance Calculation Worksheet*" title using the **Helper.AddParagraph** method that takes the parameter of the **Gehtsoft.PDFFlow.Builder.RepeatingAreaBuilder** class.

The method adds the numbered list using the **Helper.AddNumberedListToParagraph** method. It passes the array of the items using the **GetAboutAccountBalanceCalculationItems** method.
```csharp
...
The method creates the array of the the rows.
...
* Adds and sets the table with 2 columns, one column for the instructions and one column for the form with the input fields ($ ) using the **ConfigureDescriptionFormTable** method; 
...
The method sets the table. The method adds 2 columns of the specified width and sets the margin to the table.
...
* Sets the table equally using the **ConfigureDescription** method described above. The table consists of 2 columns. 
Left narrow column is for item names: A, B, C, respectively. Right wide column is for instructions. This is done so that the text of the instructions are indented from the left margin by the width of the item name.
* Adds items of the instructions using the **AddDescriptionRow** method for each item. 
The item name, or space in case on no item name, and the action to add the item of the text instructions are passed to the method.  This is done using:
...
* Sets the minimum height of the row using the value passed in the parameter;
* Fills the left cell of the row using the item name passed in the parameter;
* Calls the action passed in the parameter for the right cell.
...
###### The AddDescriptionToCell method
The method sets the cell specifying its font and dotted border.
The method adds the paragraphs for each string from the array passed in the parameter in the **foreach** cycle.
...
The method adds the paragraphs for each string from the array passed in the parameter in the **for** cycle.
The method sets the Bold font for the first paragraph.
...
###### The AddWithdrawalsDescriptionToCell method
The method sets the cell not to have any borders.
The method adds the paragraphs for each string from the array passed in the parameter in the **foreach** cycle.
It is used by the method:
...
* Fills the first row by headers; 
* Adds 26 empty row in the **for** cycle;
* Adds and sets the last row to make it without borders;
* Adds and sets the last row to make the cells of the row, except the last one, without borders;
* Add the "*Total amount $*" text to the penultimate cell of the last row.
...
All methods add the text paragraphs to the containers passed to them in the same way.

The methods differ in the type of the first parameter.

The parameter can be:
* The **cellBuilder** - an object of the**Gehtsoft.PDFFlow.Builder.TableCellBuilder** class.
* The **sectionBuilder** - an object of the **Gehtsoft.PDFFlow.Builder.SectionBuilder** class.
* The **builder** - an object of the **Gehtsoft.PDFFlow.Builder.RepeatingAreaBuilder** class.

In addition to the described above, the methods can have the following parameters: 
* The **topMagin** - the optional parameter. float.  The margin value above the paragraph.
* The **text** - the text of the paragraph.
* The **paragrpaphFont** - the font of the text.
* The **bottomMagin** - the optional parameter. float.  The margin value below the paragraph.
...
The method returns the date in text format in accordance with the passed format or a passed default value when null is passed instead of date. 

The parameters:

* The **date** - the object of the DateTime class or null;
* The **format** - the date display format, string;
* The **forNull** - the returned value if null is passed in the **date** parameter.

###### The ReplaceNullStr method

The method returns the passed string or a passed default value when null is passed instead of date.

The parameters:

* The **value** - the value that must be returned if it is not null, string;
* The **forNull** - the returned value if null is passed in the **value** parameter.
...
* The **value** - the value, double, to be converted to string;
* The **forZero** - the returned value if NaN or 0 пis passed in the **value** parameter;
* The **currencyPrefix** - optional prefix, default is an empty string.
...
* The **value** - the value, int, to be converted to string;
* The **forZero** - the returned value if 0 is passed in the **value** parameter.

###### The AddFormRow method

The method adds the row in the text__________  form to the table's cell.

The parameters:

* The **cellBuilder** - theobject of the**Gehtsoft.PDFFlow.Builder.TableCellBuilder** class;
* The **text** - the optional prefix, string, default is "$".
...
The method adds the row in the text__________ form to the table's cell placing the cell to the rectangle. 
...
The method adds the numbered list using the array of the strings passed to it in the **items** parameter.
The method adds the paragraph for each item of the array and sets the **numbered** type of the list item using the **Gehtsoft.PDFFlow.Builder.ParagraphBuilder.SetListNumbered** method. 
